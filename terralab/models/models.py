...
class Sample(models.Model):
    ...
    def compute_terralab_next_action(self, order_terralab_status):
        if not self.sample_type:
            return 'Set sample type for sample %s' % (self.name_get()[0][1])
        if not self.serial_number:
            return 'Set serial number for sample %s' % (self.name_get()[0][1])
        if len(self.tests) <= 0:
            return 'Add tests for sample %s' % (self.name_get()[0][1])
    ...
    def compute_terralab_next_action(self, order_terralab_status):
        if order_terralab_status in ('submitted', 'accepted', 'rejected'):
            if self.value == '' or self.value == False:
                return 'Add value for test variable %s' % (self.name)
...
class Test(models.Model):
    ...
    def compute_terralab_next_action(self, order_terralab_status):
        ...
        if len(self.test_variables) <= 0:
            return 'Add test variables for test %s' % (self.name_get()[0][1])
